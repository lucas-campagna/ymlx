use crate::error::{YmxError, SourceLocation};
...
impl ErrorReporter {
    ...
    pub fn report_error(&self, error: &YmxError) {
        ...
        let (message, location) = match error {
            YmxError::ParseError { message, location } => {
                (message.clone(), location.clone())
            },
            YmxError::YamlSyntaxError { message, line, column, .. } => {
                (message.clone(), SourceLocation::new("", *line, *column, 0))
            },
            YmxError::ComponentNotFound { component_id } => {
                (format!("Component not found: {}", component_id), SourceLocation::new("", 0, 0, 0))
            },
            YmxError::InvalidPropertyReference { property } => {
                (format!("Invalid property reference: {}", property), SourceLocation::new("", 0, 0, 0))
            },
            YmxError::ExecutionTimeout { limit } => {
                (format!("Execution timeout: exceeded {}", limit), SourceLocation::new("", 0, 0, 0))
            },
            YmxError::MemoryLimitExceeded { used, limit } => {
                (format!("Memory limit exceeded: used {}, limit {}", used, limit), SourceLocation::new("", 0, 0, 0))
            },
            YmxError::SecurityViolation { violation } => {
                (format!("Security violation: {}", violation), SourceLocation::new("", 0, 0, 0))
            },
            YmxError::InterpreterError { error } => {
                (format!("Interpreter error: {}", error), SourceLocation::new("", 0, 0, 0))
            },
            YmxError::IoError(_) => {
                ("I/O error occurred".to_string(), SourceLocation::new("", 0, 0, 0))
            },
            YmxError::ConfigurationError { message } => {
                (format!("Configuration error: {}", message), SourceLocation::new("", 0, 0, 0))
            },
        };
    ...
    pub fn report_errors(&self, errors: &[YmxError]) {
        ...
        self.logger.error(&format!("Found {} error(s)", errors.len()));
        ...
        for error in errors {
            match error {
                YmxError::YamlSyntaxError { .. } => {
                    suggestions.push("Check YAML syntax: ensure proper indentation and quote usage");
                },
                YmxError::InvalidPropertyReference { .. } => {
                    suggestions.push("Verify property names are spelled correctly and defined in context");
                },
                YmxError::ComponentNotFound { .. } => {
                    suggestions.push("Check component name spelling and ensure component file is included");
                },
                ...
                YmxError::ExecutionTimeout { .. } => {
                    suggestions.push("Consider optimizing component logic or increasing timeout limits");
                },
    ...
    pub fn format_error_for_cli(&self, error: &YmxError) -> String {
        match error {
            ...
            YmxError::YamlSyntaxError { message, line, column, .. } => {
                format!("YAML syntax error at line {}, column {}: {}", line, column, message)
            },
            YmxError::ComponentNotFound { component_id } => {
            ...
            YmxError::MemoryLimitExceeded { used, limit } => {
                format!("Memory limit exceeded: used {} bytes, limit {} bytes", used, limit)
            },
            YmxError::SecurityViolation { violation } => {
