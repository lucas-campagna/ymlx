use crate::models::{
    YMXComponent, ComponentValue, ExecutionEnvironment, Value,
    SecurityPolicy, Interpreter
};
use crate::error::{YmxError, Result};
use crate::interpreters::{InterpreterEngine, create_interpreter};
use crate::processing::PropertySubstitution;
...
pub struct ExecutionEngine {
    ...
    substitution_engine: PropertySubstitution,
}
...
impl ExecutionEngine {
    pub fn new() -> Self {
        Self {
            logger: get_logger(),
            substitution_engine: PropertySubstitution::new(),
        }
    }
    ...
    fn execute_processing_context(
        ...
        let interpreter = metadata.interpreter.as_ref().unwrap_or(&Interpreter::JavaScript);
    ...
    fn execute_template(&self, pattern: &str, context: &HashMap<String, Value>) -> Result<Value> {
    ...
    pub fn execute_with_dependencies(
        &self,
        components: &HashMap<String, YMXComponent>,
        root_component: &str,
    ...
    pub fn validate_execution(&self, env: &ExecutionEnvironment) -> Result<()> {
        // Check memory limits
        if env.memory_limit > env.security_policy.max_memory {
            return Err(YmxError::SecurityViolation {
                violation: format!("Memory limit {} exceeds allowed {}", env.memory_limit, env.security_policy.max_memory),
            });
        }
        
        // Check time limits
        if env.time_limit > env.security_policy.max_execution_time {
            return Err(YmxError::SecurityViolation {
                violation: format!("Time limit {:?} exceeds allowed {:?}", env.time_limit, env.security_policy.max_execution_time),
            });
        }
        
        // Check API access
        if !env.security_policy.allow_file_access {
            // In a real implementation, we would prevent file access
            self.logger.debug("File access restricted by security policy");
        }
        
        if !env.security_policy.allow_network_access {
            // In a real implementation, we would prevent network access
            self.logger.debug("Network access restricted by security policy");
        }
